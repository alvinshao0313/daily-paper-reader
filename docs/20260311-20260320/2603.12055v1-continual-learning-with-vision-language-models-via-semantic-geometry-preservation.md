...
tags: ["query:mc"]
score: 7.0
evidence: 预训练视觉语言模型的持续学习
tldr: 本研究针对预训练视觉语言模型在持续学习中因几何畸变导致的灾难性遗忘问题，提出了SeGP-CL框架。该方法通过双目标投影梯度下降构建对抗锚点以探测易漂移区域，并结合锚点引导的跨模态几何蒸馏与文本语义正则化来稳定模型结构。实验证明，SeGP-CL在五个基准测试中均显著提升了模型的稳定性和前向迁移能力，达到了当前最先进的性能水平。
selection_source: fresh_fetch
motivation: 现有方法在适应新任务时未能显式保护预训练的跨模态语义几何，导致新任务监督引发严重的几何畸变和遗忘。
method: 提出SeGP-CL框架，利用对抗锚点探测漂移区域，并通过几何蒸馏、文本语义正则化及双路径推理来保持跨模态结构。
result: 在五个持续学习基准测试上的广泛实验表明，SeGP-CL在保持语义几何稳定性的同时，一致性地超越了现有SOTA方法。
conclusion: 通过显式维护跨模态语义几何的完整性，可以有效缓解视觉语言模型在无样本持续学习场景下的灾难性遗忘。
---
