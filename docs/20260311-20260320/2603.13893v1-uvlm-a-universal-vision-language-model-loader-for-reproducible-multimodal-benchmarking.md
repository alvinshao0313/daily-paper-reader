...
## 摘要
视觉语言模型 (VLMs) 已成为图像理解任务的强大工具，但由于不同模型家族之间显著的架构异构性，其实际部署仍面临阻碍。本文介绍了 UVLM (Universal Vision-Language Model Loader)，这是一个基于 Google Colab 的框架，为在自定义图像分析任务上加载、配置和基准测试多种 VLM 架构提供了统一接口。UVLM 目前支持两个主要的模型家族——LLaVA-NeXT 和 Qwen2.5-VL，它们在视觉编码、分词（tokenization）和解码策略上存在根本差异。该框架通过单一推理函数抽象了这些差异，使研究人员能够使用相同的提示词和评估协议来比较模型。关键特性包括：支持四种响应类型（数值、类别、布尔值、文本）的多任务提示词构建器；基于多次重复推理中多数投票的共识验证机制；灵活的 Token 预算（最高 1,500 个 Token），允许用户通过提示词工程设计自定义推理策略；以及内置的用于基准测试的思维链（chain-of-thought）参考模式。UVLM 旨在实现可重复性、易用性和可扩展性，因此可以使用消费级 GPU 资源在 Google Colab 上免费部署。本文还利用包含 120 张街景图像的语料库，首次对不同 VLM 在推理复杂度不断增加的任务上的表现进行了基准测试。
