...
## 摘要
新兴的生成式人工智能（Generative AI）部署越来越多地在去中心化且异构的边缘设备上执行推理，而非在单个受信任的服务器上。在此类环境下，单个设备的故障或异常行为可能会中断整个推理过程，使得传统的尽力而为（best-effort）点对点路由机制不再适用。因此，协调分布式生成式推理需要能够明确考虑参与节点之间的可靠性、性能波动以及信任度的机制。在本文中，我们提出了 G-TRAC，这是一个信任感知协调框架，它将算法路径选择与系统级协议设计相结合，以确保鲁棒的分布式推理。首先，我们将路由问题建模为“风险受限最短路径”（Risk-Bounded Shortest Path）计算，并引入了一种结合信任底线剪枝与 Dijkstra 搜索的多项式时间解法，在实际边缘规模下实现了亚毫秒级的平均路由延迟，在大规模场景下也能保持在 10 毫秒以下。其次，为了在动态环境中支持路由逻辑的运行，该框架采用了“混合信任架构”（Hybrid Trust Architecture），在稳定的锚点处维护全局信誉状态，同时通过后台同步向边缘节点分发轻量级更新。在商用设备组成的异构测试平台上的实验评估表明，G-TRAC 显著提高了推理完成率，有效隔离了不可信节点，并且在节点故障和网络分区的情况下仍能维持鲁棒执行。
