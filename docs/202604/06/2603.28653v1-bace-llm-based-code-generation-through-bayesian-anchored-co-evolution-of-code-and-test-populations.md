...
score: 8.0
evidence: 基于LLM的规划与推理代码生成
tldr: 本文针对大模型代码生成中自动测试反馈不可靠的问题，提出了BACE框架。BACE将代码生成建模为贝叶斯协同演化过程，通过维护代码和测试两个种群，并利用贝叶斯更新机制处理测试中的噪声信号。该方法以公开示例为锚点防止演化漂移，有效解决了传统方法中错误测试误导代码生成的问题。在LiveCodeBench v6上的实验表明，BACE显著提升了各类模型的代码生成能力。
selection_source: carryover_cache
motivation: 传统的自动测试反馈机制因将不可靠的生成测试视为绝对真理，容易导致代码生成陷入错误验证的死循环。
method: 提出贝叶斯锚定协同演化框架，通过代码与测试种群的相互迭代更新和贝叶斯信念分布来过滤噪声并优化搜索。
result: 在最新的LiveCodeBench v6基准测试中，BACE在闭源大模型和开源小模型上均实现了超越现有方法的性能表现。
conclusion: 研究证明通过贝叶斯方法对生成测试进行噪声建模和协同演化，是提升大模型代码生成鲁棒性的有效途径。
---

## 摘要
大语言模型（LLMs）在代码生成方面展现出了卓越的能力。虽然交互式反馈循环可以提升性能，但编写有效的测试并非易事。早期的多智能体框架（如 AgentCoder）实现了这一过程的自动化，但将生成的测试视为绝对的真值（ground truth）。这种方法较为脆弱：错误的代码经常能通过有缺陷或琐碎的测试，而有效的解决方案往往为了满足错误的断言而被降级。针对这一局限性，较新的方法在很大程度上放弃了测试生成，转而采用基于示例的规划和推理。然而，我们认为，如果将生成的测试建模为受贝叶斯更新引导的有噪传感器（noisy sensors），它们仍然是宝贵的信号。为此，我们提出了 BACE（贝叶斯锚定协同演化），该框架将代码合成重新表述为一个贝叶斯协同演化过程，其中代码和测试种群在信念分布的引导下共同演化，而信念分布则根据有噪交互证据进行相互更新。通过将搜索锚定在极少数公开示例上，BACE 防止了自我验证循环中常见的协同演化漂移。在 LiveCodeBench v6（2025 年 3 月后）上的广泛评估表明，BACE 在闭源模型和开源小语言模型上均取得了优异的性能。
