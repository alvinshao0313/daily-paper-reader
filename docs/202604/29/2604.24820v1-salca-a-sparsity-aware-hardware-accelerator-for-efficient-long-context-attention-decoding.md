...
tldr: Salca通过软硬件协同设计和量化技术加速了长文本大语言模型的注意力解码。
source: arxiv
...
motivation: 针对长文本大语言模型的超低精度量化与动态稀疏注意力。
...
## 摘要
长上下文提升了大语言模型的能力，但也带来了严峻的硬件挑战：计算和内存占用随序列长度线性增长。特别是解码阶段需要持续访问海量的 KV 缓存，极大地增加了带宽和计算压力。现有的加速器主要针对短上下文进行设计和评估，在处理长上下文时性能会显著下降。为了弥补这一差距，我们识别了主要瓶颈，并通过软硬件协同设计提出了一种用于长上下文注意力解码的硬件加速器。在软件方面，我们提出了双压缩动态稀疏注意力。它结合了超低精度量化与特征稀疏性，以最小化预测开销。一种硬件友好的近似 Top-K 选择进一步将过滤复杂度从 $O(n \log k)$ 降低到 $O(n)$。在硬件方面，我们深度优化了计算和内存访问，以解决稀疏注意力与长上下文之间复杂相互作用带来的瓶颈，并建立了一个性能模型来推导最优的协同设计方案。最终的硬件采用了全流水线并行架构，即使对于长序列也能实现 $O(n)$ 的效率。实验表明，与 A100 相比，我们的设计实现了 3.82 倍的加速和 74.19 倍的能效。与最先进的（SOTA）加速器相比，这是首个高效支持长上下文推理的 ASIC 加速器，吞吐量至少提高 3.5 倍，能效提高 2.08 倍。
