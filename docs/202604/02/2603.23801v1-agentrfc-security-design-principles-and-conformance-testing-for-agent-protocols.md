...
score: 8.0
evidence: 自主智能体协议的安全设计原则
tldr: 针对AI智能体协议缺乏系统性安全框架的问题，本文提出AgentRFC。主要贡献包括：定义了六层智能体协议栈架构，建立了包含11项安全原则的不可知安全模型（AASM），并开发了AgentConform一致性检查工具。通过TLA+形式化验证和SDK实机测试，揭示了现有协议在凭证管理、授权执行及协议组合安全性方面的显著缺陷。
selection_source: fresh_fetch
motivation: 随着AI智能体协议的大规模部署，业界亟需一套系统性的安全框架来规范其跨信任边界的能力发现与任务委派。
method: 提出了六层协议栈模型和11项安全原则，并开发了将协议规范转化为TLA+模型进行验证并与SDK实机比对的工具AgentConform。
result: 发现了现有代表性协议在凭证生命周期、同意执行、审计完整性以及协议组合安全性方面存在普遍缺陷。
conclusion: 本研究强调了协议组合安全的重要性，并为构建更安全的自主智能体生态系统提供了形式化方法和测试工具。
---

## 摘要
AI 智能体协议（包括 MCP、A2A、ANP 和 ACP）使自主智能体能够跨越信任边界发现能力、委派任务并组合服务。尽管部署规模巨大（仅 MCP 每月 SDK 下载量就超过 9700 万次），但目前尚不存在针对这些协议的系统性安全框架。我们提出了三项贡献：第一，智能体协议栈（Agent Protocol Stack），这是一个 6 层架构模型，定义了完整的智能体协议在每一层必须规范的内容，类似于 OSI 栈的 ITU-T X.800；第二，通用智能体安全模型（Agent-Agnostic Security Model），将 11 条安全原则形式化为 TLA+ 不变量，每条原则都标记了属性分类（规范强制、规范推荐、AASM 加固、APS 完整性），用以区分协议不一致性与框架施加的安全要求；第三，AgentConform，一个两阶段一致性检查器，它 (i) 从协议规范中提取规范性条款，转化为具有明确协议/环境/对手动作分离的有类型协议中间表示（Protocol IR），(ii) 将该 IR 编译为 TLA+ 模型并根据 AASM 不变量进行模型检测，然后 (iii) 在真实的 SDK 实现上重放反例轨迹以确认发现。我们引入了组合安全性（Composition Safety, CS）原则：对于单个协议成立的安全属性，在协议通过共享基础设施进行组合时可能会失效。我们通过五种协议组合模式的形式化模型证明了这一点，揭示了单个协议分析无法检测到的跨协议设计缺陷。对代表性智能体协议的初步应用揭示了在凭证生命周期、授权执行、审计完整性和组合安全性方面反复出现的缺陷。部分发现正在进行协调披露；完整的评估细节将在最终版本中发布。
