...
title_zh: 一种用于在共享内存网络中加速大语言模型推理的以交换机为中心的在网架构
...
score: 6.0
evidence: 通过网络内计算加速LLM推理
tldr: 针对大模型推理中的通信瓶颈，本文提出首个面向共享内存网络的以交换机为中心的网络内计算架构SCIN。SCIN通过引入交换机内加速器（ISA）和协同设计的通信结构，消除了传统NVLS架构中冗余的数据移动，并支持网络内量化（INQ）技术。实验表明，SCIN显著降低了All-Reduce延迟并提升了带宽，在LLaMA-2模型上实现了显著的推理加速。
selection_source: fresh_fetch
motivation: 现有的以加速器为中心的网络内计算架构（如NVLS）存在冗余通信开销，且无法支持量化等非内存语义算子的卸载。
method: 提出SCIN架构，通过在交换机中集成可主动发起内存操作的加速器（ISA）并结合网络内量化技术，优化All-Reduce操作。
result: 在多FPGA原型系统上，SCIN将All-Reduce速度提升高达8.7倍，并使LLaMA-2的首字生成时间和每Token生成时间分别缩短了1.74倍和1.34倍。
conclusion: SCIN通过以交换机为中心的设计有效解决了共享内存网络中的通信瓶颈，是加速大模型推理的高效方案。
---

## 摘要
以 NVLink Sharp (NVLS) 为代表的在网计算技术，通过将 All-Reduce 等集合通信操作卸载到交换机，为解决大语言模型 (LLM) 推理中的通信瓶颈提供了一种极具前景的方法。然而，NVLS 以加速器为中心的架构存在两个根本局限性：1) 它依赖 GPU 的 load 指令来触发归约操作，这意味着在交换机中归约后的数据必须额外传输回发起请求的 GPU，而不能直接广播，从而引入了不必要的通信开销；2) 受限于架构约束，NVLS 无法卸载那些不能分解为内存语义指令的算子，例如本研究中提出的在网量化 (INQ)。因此，NVLS 中的 All-Reduce 必须以 FP16/BF16 精度运行，导致了大量的带宽浪费。为了解决这些局限性，我们提出了 SCIN，这是首个针对 AI 加速器共享内存网络的以交换机为中心的在网架构，能够实现低延迟且高带宽的 All-Reduce。具体而言，我们引入了一种交换机内加速器 (ISA)，它能够发起用于在网处理的内存语义操作，并配合协同设计的通信织网，从而产生极低的协议开销。通过消除冗余的数据移动，SCIN 提供了比 NVLS 更低的 All-Reduce 延迟。此外，通过在 ISA 中集成量化模块，SCIN 实现了 All-Reduce 的在网量化 (INQ)，将其精度降低至 8 位，在精度损失可忽略不计的情况下使带宽几乎翻倍。我们还在多 FPGA 系统上展示了 SCIN 的原型，以证明其可行性和有效性。实验结果表明，我们的设计在小消息和大消息下分别将 All-Reduce 加速了高达 8.7 倍和 3.8 倍，在 LLaMA-2 模型上实现了高达 1.74 倍的首字生成延迟 (TTFT) 加速和 1.34 倍的每 token 生成延迟 (TPOT) 加速。
