...
score: 6.0
evidence: 无反向传播的自适应方法以提升运行效率
tldr: "本研究提出PACE，一种无需反向传播的持续测试时自适应系统。针对现有无导数方法在运行效率与学习能力间难以平衡的问题，PACE通过CMA-ES算法结合Fastfood投影，在低维子空间内优化归一化层的仿射参数。此外，引入自适应停止准则和领域专用向量库以减少冗余计算。实验表明，PACE在多个基准测试中达到SOTA精度，且运行时间比同类方法缩短50%以上。"
selection_source: fresh_fetch
motivation: 现有的无反向传播测试时自适应方法在处理持续分布偏移时，难以兼顾模型更新的表达能力与计算效率。
method: 采用CMA-ES进化策略结合Fastfood投影在低维子空间优化高维仿射参数，并利用停止准则和向量库提升效率。
result: "在多个持续分布偏移基准上实现了最先进的准确率，同时将运行时间降低了50%以上。"
conclusion: PACE证明了通过子空间优化和冗余消除，可以在无需反向传播的情况下实现高效且高性能的持续测试时自适应。
---
