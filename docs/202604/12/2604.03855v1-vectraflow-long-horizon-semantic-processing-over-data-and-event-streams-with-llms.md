...
title_zh: VectraFlow：基于大语言模型的长程数据流与事件流语义处理
authors: "Shu Chen, Junhan Liu, Deepti Raghavan, Ugur Cetintemel"
date: 2026-04-04
pdf: "https://arxiv.org/pdf/2604.03855v1"
tags: ["query:llm-agent"]
score: 7.0
evidence: 长程语义处理与大模型驱动的数据流执行
tldr: VectraFlow是一个语义流数据处理引擎，旨在解决现有LLM框架缺乏状态化推理以及传统复杂事件处理（CEP）系统无法处理非结构化文本的问题。它通过将LLM能力集成到关系算子（如过滤、聚合、窗口等）中，并结合NFA时序规则匹配，实现了对非结构化文本流的长周期、状态化语义处理。该系统支持在吞吐量和准确性之间进行权衡，适用于临床文档流等复杂场景。
selection_source: fresh_fetch
motivation: 现有的LLM框架多为无状态的一次性处理，而传统的复杂事件处理系统难以应对非结构化文本流中的长周期语义推理需求。
method: 提出VectraFlow引擎，将LLM驱动的语义算子与基于NFA的时序规则匹配相结合，构建支持过滤、聚合及窗口操作的语义数据流。
result: 实现了从原始文本到匹配事件队列的端到端时序模式检测，并提供了可配置的吞吐量与准确性权衡方案。
conclusion: VectraFlow成功将复杂事件处理提升至语义层面，为非结构化数据流的长周期状态化推理提供了高效的解决方案。
---

## 摘要
监测连续数据中的有意义信号，日益需要对非结构化流进行长程、有状态的推理。然而，目前的 LLM 框架仍保持无状态和单次触发模式，而传统的复杂事件处理（CEP）系统虽然具备时序模式检测能力，但通常假设事件流是结构化且有类型的，导致无法处理非结构化文本。我们展示了 VectraFlow，一个旨在弥补这两项空白的语义流式数据流引擎。VectraFlow 通过在自由文本流上引入 LLM 驱动的执行能力，扩展了传统的数据库关系算子，提供了一套连续语义算子——包括过滤（filter）、映射（map）、聚合（aggregate）、连接（join）、分组（group-by）和窗口（window）——每种算子在基于 LLM、基于嵌入（embedding）以及混合实现中均具有可配置的吞吐量与准确率权衡。在此基础上，语义事件模式算子将复杂事件处理提升至非结构化文档流，结合了基于 LLM 的事件提取与基于 NFA（非确定有限状态自动机）的时序规则匹配，实现了对语义事件序列的有状态推理。在本次演示中，用户将通过 VectraFlow 的实时查询界面，在临床文档流上构建语义流水线。与会者可以将自然语言意图编译为可执行的算子图，检查中间状态输出，并观察从原始文本到匹配事件队列（event cohorts）的端到端时序模式检测。
