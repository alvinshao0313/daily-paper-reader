...
tldr: SAGAI-MID 是一款基于 FastAPI 的生成式 AI 中间件，旨在解决分布式系统中异构服务（如 REST、GraphQL、IoT）间的模式不匹配问题。它通过 LLM 实现运行时的动态检测与转换，采用五层流水线架构和三层安全保障，将传统的设计时互操作性策略转化为运行时能力。实验表明其最高准确率达 0.90，且代码生成策略优于直接转换，为软件架构引入 LLM 作为运行时组件提供了实践参考。
selection_source: fresh_fetch
motivation: 传统的静态适配器需要手动编码且无法处理运行时的未知模式组合，难以应对现代分布式系统中频繁变动的异构服务互操作挑战。
method: 提出一种包含混合检测、双重解析策略（直接转换与代码生成）及三层安全保障的五层 LLM 驱动中间件架构。
result: 在 10 个互操作性场景的评估中，系统最高准确率达 0.90，且代码生成策略在性能上优于直接转换，同时发现模型成本与准确率并不成正比。
conclusion: LLM 可以有效地作为运行时架构组件实现动态互操作性，为软件架构师提供了低成本、高灵活性的系统集成新方案。
---

## 摘要
现代分布式系统集成了异构服务、具有不同模式（schema）版本的 REST API、GraphQL 端点以及具有专有负载的物联网（IoT）设备，这些系统长期受到模式不匹配问题的困扰。传统的静态适配器需要为每一对模式进行手动编码，且无法在运行时处理新的组合。我们提出了 SAGAI-MID，这是一种基于 FastAPI 的中间件，利用大语言模型（LLM）在运行时动态检测并解决模式不匹配问题。该系统采用五层流水线：混合检测（结构差异对比加 LLM 语义分析）、双重解决策略（单次请求 LLM 转换和 LLM 生成的可重用适配器代码）以及三层保障栈（验证、集成投票、基于规则的回退）。我们通过 Bass 等人的互操作性策略来构建该架构，将其从设计时的人工产物转变为运行时能力。我们在涵盖 REST 版本迁移、IoT 到分析平台的桥接以及 GraphQL 协议转换的 10 个互操作性场景中，对来自两个供应商的 6 个 LLM 进行了评估。性能最佳的配置达到了 0.90 的 pass@1 准确率。CODEGEN 策略的表现始终优于 DIRECT 策略（平均 pass@1 为 0.83 对比 0.77），而不同模型之间的成本差异超过 30 倍，且准确率并未随之成比例增长；最准确的模型反而是最便宜的。我们讨论了软件架构师将 LLM 作为运行时架构组件所带来的启示。
