...
## 摘要
自动化仓库和物流中的多智能体协作通常被建模为多智能体路径规划（MAPF）问题。闭环 MAPF 算法通过仅规划下一步行动并进行在线重规划来提高可扩展性，但这种有限时域视角可能具有局限性，且难以维持全局保证并利用组合结构。这一问题在随时闭环冲突搜索（ACCBS）中尤为明显，该算法在动态扩展的有限时域上应用冲突搜索（CBS），但在有限的计算预算下，在密集实例中可能会以较短的活动前缀终止。我们引入了凭证轨迹及其相关的车队预算，作为过滤闭环更新的通用机制。凭证提供了一个无冲突的备选方案和剩余成本的单调上界；仅接受改进凭证的更新可确保完备性。相同的预算信息诱导了一种受预算限制的分解，从而实现了跨时间步的全局、可继承分解。在 ACCBS 上实例化该框架得到了凭证驱动的冲突搜索（CDCBS）。在基准地图上的实验表明，CDCBS 比 ACCBS 具有更一致的解质量，特别是在密集场景下，同时所提出的分解方法减小了有效群体规模。
