...
score: 7.0
evidence: 多模态大语言模型（MLLM）的无监督自我演化
...
motivation: 现有的自进化方法依赖多数投票选择伪标签，容易受模型固有偏见影响而导致推理路径质量下降。
method: 提出了包含回溯重推理机制、平滑频率奖励和视觉语义扰动的CSRS框架，以优化推理路径探索和奖励校准。
result: 在MathVision等基准测试中显著提升了Qwen2.5-VL-7B的性能，并在几何任务的无监督自进化中取得SOTA结果。
conclusion: CSRS通过改进反馈信号和探索机制，有效稳定了MLLM的无监督自进化过程，增强了模型的数学逻辑推理能力。
---

## 摘要
在多模态大语言模型（MLLMs）的无监督自我演化中，后训练阶段反馈信号的质量对于稳定且有效的学习至关重要。然而，现有的自我演化方法主要依赖多数投票（majority voting）来选择出现频率最高的输出作为伪金标准答案，这可能源于模型的固有偏见，而无法保证推理路径的客观正确性。为了应对这种退化，我们提出了 MLLM 自我演化中的连续平滑回溯重采样（CSRS）。具体而言，我们引入了一种回溯重推理机制（RRM），使模型能够从锚点开始重新推理，以扩大对长尾推理路径的探索。同时，我们提出了平滑频率奖励（SFR），用连续信号取代二元奖励，并根据采样推理集中答案的频率来校准奖励。此外，结合视觉语义扰动（VSP），CSRS 确保模型优先考虑数学逻辑而非视觉表象。实验结果表明，CSRS 显著提升了 Qwen2.5-VL-7B 在 MathVision 等基准测试上的推理性能。我们在几何任务的无监督自我演化方面达到了最先进（SOTA）的水平。我们的代码可在 https://github.com/yyy195/CSRS 获取。
