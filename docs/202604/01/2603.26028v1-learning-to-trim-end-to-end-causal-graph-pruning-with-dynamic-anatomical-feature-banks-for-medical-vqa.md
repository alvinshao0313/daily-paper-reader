...
title_zh: 学习修剪：用于医疗视觉问答的动态解剖特征库端到端因果图剪枝
authors: "Zibo Xu, Qiang Li, Weizhi Nie, Yuting Su"
date: 2026-03-27
pdf: "https://arxiv.org/pdf/2603.26028v1"
tags: ["query:model-compression"]
score: 6.0
evidence: 医疗视觉问答中的因果图剪枝
tldr: 针对医疗视觉问答（MedVQA）中模型过度依赖数据集特定相关性而非诊断证据的问题，本文提出了一种可学习因果修剪（LCT）框架。该框架通过动态解剖特征库（DAFB）捕捉全局原型，并利用可微分修剪模块抑制与全局原型高度相关的冗余特征，从而强化实例特有的因果信号。实验证明该方法在多个数据集上显著提升了模型的鲁棒性和泛化能力。
selection_source: fresh_fetch
motivation: 现有的MedVQA模型往往依赖于数据集中的虚假相关性而非真实的诊断证据，且现有的因果去偏方法多为静态或事后处理。
method: 提出LCT框架，利用动量更新的动态解剖特征库捕捉全局规律，并通过可微分修剪模块自适应地抑制虚假相关性。
...
## 摘要
医疗视觉问答（MedVQA）模型通常由于依赖于特定数据集的相关性（如重复出现的解剖模式或问题类型规律），而非真实的诊断证据，从而表现出有限的泛化能力。现有的因果方法通常被实现为静态调整或事后修正。为解决这一问题，我们提出了一个可学习因果修剪（LCT）框架，将因果剪枝集成到端到端优化中。我们引入了通过动量机制更新的动态解剖特征库（DAFB），以捕获频繁出现的解剖和语言模式的全局原型，作为数据集级规律的近似。我们进一步设计了一个可微分修剪模块，用于估计实例级表示与全局特征库之间的依赖关系。与全局原型高度相关的特征被软抑制，而特定于实例的证据则被强调。这种可学习机制鼓励模型自适应地优先考虑因果信号而非虚假相关性。在 VQA-RAD、SLAKE、SLAKE-CP 和 PathVQA 上的实验表明，相比现有的去偏策略，LCT 持续提高了鲁棒性和泛化能力。
