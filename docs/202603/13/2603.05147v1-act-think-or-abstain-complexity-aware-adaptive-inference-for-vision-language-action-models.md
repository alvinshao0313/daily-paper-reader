...
title_zh: 行动、思考或中止：视觉-语言-动作模型的复杂度感知自适应推理
...
tldr: 针对视觉-语言-动作（VLA）模型推理延迟高且缺乏安全机制的问题，本文提出一种受人类认知启发的自适应推理框架。该框架根据感知状态的复杂度，动态将任务路由至直接执行（Act）、推理执行（Think）或主动放弃（Abstain）。研究发现视觉嵌入在识别任务复杂度方面优于语言。实验表明，该方法在LIBERO基准和真实机器人上表现优异，仅需极少数据即可实现高效的任务复杂度检测，兼顾了推理效率与系统安全性。
selection_source: fresh_fetch
motivation: 现有VLA模型推理开销大且缺乏不确定性估计，导致简单任务资源浪费及复杂或异常任务下的潜在失败。
method: 提出一种自适应框架，利用视觉语言骨干网络的潜在嵌入进行复杂度评估，将任务动态路由至不同执行模式。
result: "在LIBERO基准和真实机器人实验中，仅需5%的训练数据，视觉配置即可达到80%的F1分数，有效检测任务复杂度。"
conclusion: 该研究证明了视觉嵌入在推断任务复杂度方面的优越性，为构建高效且安全的具身智能系统提供了可靠的检测机制。
---

## 摘要
当前关于视觉-语言-动作（VLA）模型的研究主要集中在通过既有的推理技术来增强泛化能力。尽管这些改进行之有效，但不可避免地增加了计算复杂度和推理延迟。此外，这些机制通常被无差别地应用，导致在简单任务上资源分配效率低下，同时又无法提供必要的确定性估计，以防止在分布外（OOD）任务上发生灾难性失败。受人类认知启发，我们提出了一种自适应框架，根据感知状态的复杂度动态路由 VLA 的执行。我们的方法通过将潜在嵌入投影到参数化和非参数化估计器的集成中，将 VLA 的视觉-语言主干转化为一种主动检测工具。这使得系统能够立即执行已知任务（行动）、对模糊场景进行推理（思考），并在遇到显著的物理或语义异常时预先停止执行（中止）。在实证分析中，我们观察到一种现象：由于语言的语义不变性，仅凭视觉嵌入在推断任务复杂度方面表现更优。在 LIBERO 和 LIBERO-PRO 基准测试以及真实机器人上的评估结果显示，我们的纯视觉配置仅使用 5% 的训练数据即可达到 80% 的 F1 分数，证明其是一种可靠且高效的任务复杂度检测器。
