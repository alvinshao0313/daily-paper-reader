...
title_zh: 频率感知且误差受限的缓存机制，用于加速扩散 Transformer
authors: Guandong Li
date: 2026-03-05
...
tldr: 针对扩散Transformer（DiT）推理成本高的问题，本文提出SpectralCache。该框架识别了DiT去噪过程中时间、深度和特征维度的非均匀性，通过时间步感知调度、累积误差预算和频率分解缓存技术，在保持图像质量的同时显著提升推理速度，是一种无需训练、即插即用的加速方案。
selection_source: fresh_fetch
motivation: 现有的DiT缓存方法忽略了去噪过程中时间、深度和特征维度的非均匀性，导致加速效率和质量平衡受限。
method: 提出SpectralCache框架，包含时间步感知动态调度（TADS）、累积误差预算（CEB）和频率分解缓存（FDC）三大核心组件。
result: "在FLUX.1-schnell模型上实现了2.46倍的加速，在保持与TeaCache相当的图像质量下，速度比其提升了16%。"
conclusion: SpectralCache通过精细化的误差控制和频率感知策略，为DiT模型提供了一种高效、通用的无损加速方案。
---

## 摘要
扩散 Transformer (DiT) 已成为高质量图像和视频生成的主流架构，但其迭代去噪过程在推理阶段产生了巨大的计算开销。现有的缓存方法通过跨时间步重用中间计算来加速 DiT，但它们存在一个共同的局限性：将去噪过程在时间、深度和特征维度上视为均匀的。在这项工作中，我们确定了 DiT 去噪中非均匀性的三个正交轴：(1) 时间——对缓存误差的敏感度在去噪轨迹上变化剧烈；(2) 深度——连续的缓存决策会导致级联的近似误差；(3) 特征——隐藏状态的不同组件表现出异构的时间动态。基于这些观察，我们提出了 SpectralCache，这是一个统一的缓存框架，包含时间步感知动态调度 (TADS)、累积误差预算 (CEB) 和频率分解缓存 (FDC)。在 512x512 分辨率的 FLUX.1-schnell 上，SpectralCache 实现了 2.46 倍的加速，LPIPS 为 0.217，SSIM 为 0.727，在速度上比 TeaCache (2.12x, LPIPS 0.215, SSIM 0.734) 提升了 16%，同时保持了相当的生成质量（LPIPS 差异 < 1%）。我们的方法无需训练、即插即用，且与现有的 DiT 架构兼容。
