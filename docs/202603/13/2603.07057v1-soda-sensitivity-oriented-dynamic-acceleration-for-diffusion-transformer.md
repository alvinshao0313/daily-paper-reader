...
title_zh: SODA：面向敏感度的扩散 Transformer 动态加速方法
...
tldr: 针对扩散Transformer（DiT）推理效率低的问题，本文提出SODA，一种基于敏感度的动态加速方法。现有方法多采用固定启发式策略，难以捕捉细粒度的敏感度变化，导致生成质量下降。SODA通过离线建模时间步、层和模块的敏感度误差，利用动态规划优化缓存间隔，并自适应调整剪枝时机与比例，在保持高加速比的同时显著提升了生成保真度，在多个主流模型上达到SOTA水平。
selection_source: fresh_fetch
motivation: 现有的DiT加速方法采用固定的缓存和剪枝策略，无法捕捉模型对加速操作的细粒度敏感度变化，导致生成质量受损且泛化性差。
method: 提出SODA方法，通过离线建立敏感度误差模型，利用动态规划优化缓存间隔，并根据敏感度自适应确定剪枝时机和比例。
result: 在DiT-XL/2、PixArt-α和OpenSora等模型上的实验表明，SODA在可控的加速比下实现了最先进的生成保真度。
conclusion: SODA通过细粒度的敏感度感知动态调整策略，有效平衡了扩散Transformer的推理速度与生成质量。
---

## 摘要
扩散 Transformer（Diffusion Transformers）已成为视觉生成领域的主导范式，但其较低的推理效率仍是阻碍其进一步发展的关键瓶颈。在常见的免训练技术中，缓存（caching）提供了较高的加速效率，但往往会损害保真度，而剪枝（pruning）则表现出相反的权衡。将缓存与剪枝相结合可以在加速与生成质量之间取得平衡。然而，现有方法通常采用固定且启发式的方案来配置缓存和剪枝策略。虽然这些方法大致遵循生成模型对加速的整体敏感度趋势，但未能捕捉到细粒度且复杂的变动，不可避免地跳过了高度敏感的计算，从而导致质量下降。此外，这类人工设计的策略泛化能力较差。为解决这些问题，我们提出了 SODA，一种面向敏感度的动态加速方法，该方法基于细粒度敏感度自适应地执行缓存和剪枝。SODA 构建了一个跨时间步、层和模块的离线敏感度误差建模框架，以捕捉对不同加速操作的敏感度。缓存间隔通过以敏感度误差为代价函数的动态规划进行优化，从而最大限度地减少缓存对模型敏感度的影响。在剪枝和缓存重用过程中，SODA 自适应地确定剪枝时机和比例，以保留高度敏感 token 的计算，显著提升了生成保真度。在 DiT-XL/2、PixArt-α 和 OpenSora 上的大量实验表明，SODA 在可控的加速比下实现了最先进的生成保真度。我们的代码已公开发布于：https://github.com/leaves162/SODA。
