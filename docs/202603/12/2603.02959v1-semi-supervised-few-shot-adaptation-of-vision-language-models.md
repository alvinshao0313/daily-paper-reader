...
score: 8.0
evidence: 视觉语言模型的适配
tldr: "本研究针对视觉语言模型（VLM）在医疗影像等少样本场景下的适配问题，提出了一种半监督学习方法。针对极低样本量下类别不平衡导致的性能下降，该方法通过引入高效的半监督求解器，利用文本信息的伪标签来传播未标记数据的信息。实验表明，该方法在保持性能的同时，能将标注成本降低50%以上，为低预算下的模型适配提供了有效方案。"
selection_source: carryover_cache
motivation: 医疗影像专家标注成本高昂，且在极低样本适配时，类别不平衡问题会严重影响视觉语言模型的性能。
method: 提出一种高效的半监督求解器，在少样本适配过程中利用文本引导的伪标签来传播未标记数据的信息。
result: "该方法在低样本机制下成功将标注工作量减少了50%以上，并有效缓解了类别不平衡带来的负面影响。"
conclusion: 通过结合未标记数据和文本先验，半监督适配能显著提升视觉语言模型在专业领域任务中的效率和实用性。
---
