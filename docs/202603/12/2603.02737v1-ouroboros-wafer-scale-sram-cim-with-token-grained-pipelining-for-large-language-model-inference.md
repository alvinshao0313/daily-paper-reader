...
title_zh: Ouroboros：具有 Token 粒度流水线的用于大语言模型推理的晶圆级 SRAM 存内计算架构
...
## 摘要
传统的 LLM 推理架构由于存储层级间频繁的数据移动，面临高能耗和高延迟的问题。我们提出了 Ouroboros，一种基于晶圆级 SRAM 的存内计算（CIM）架构，该架构在原位执行所有操作，消除了片外数据迁移。为了最大化其有限的一级容量，我们引入了三项创新：Token 粒度流水线：取代序列级流水线以缓解长度变化的影响，从而提高利用率并减少激活存储；分布式动态 KV 缓存管理：将存储与计算解耦，利用碎片化的 SRAM 进行高效的 KV 存储；通信感知映射：优化整个晶圆上的核心分配，以实现局部性和容错性。实验结果表明，Ouroboros 在吞吐量和能效方面分别实现了平均 4.1 倍和 4.2 倍的提升，在 13B 模型上峰值提升分别达到 9.1 倍和 17 倍。（*由于 arXiv 通知“摘要字段不能超过 1,920 个字符”，此处显示的摘要已缩短。完整摘要请下载论文原文。）
