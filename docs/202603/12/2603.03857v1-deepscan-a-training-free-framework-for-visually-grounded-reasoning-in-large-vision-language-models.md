...
title_zh: DeepScan：一种用于大视觉语言模型视觉定位推理的免训练框架
...
## 摘要
人类能够通过识别关键线索并以自下而上的方式将其与全局上下文联系起来，从而在噪声环境中稳健地定位视觉证据并提供有据可依的回答。受此启发，我们提出了 DeepScan，这是一个结合了层级扫描（Hierarchical Scanning）、重新聚焦（Refocusing）和证据增强推理（Evidence-Enhanced Reasoning）的免训练框架，旨在实现大视觉语言模型（LVLMs）中的视觉定位推理。与现有追求完整证据单次定位的方法不同，层级扫描通过局部线索探索和多尺度证据提取，以自下而上的方式恢复证据，有效缓解了干扰性上下文的影响。随后，重新聚焦通过 LVLM 与视觉专家的协作来优化定位后的证据视图。最后，证据增强推理通过混合证据记忆聚合多粒度视图，并生成准确且可解释的答案。实验结果表明，DeepScan 在多种视觉任务中显著提升了 LVLM 的性能，尤其是在细粒度视觉理解方面。当与 Qwen2.5-VL-7B 集成时，它在 V* 评测中达到了 90.6% 的总准确率。此外，DeepScan 为不同架构和模型规模的 LVLM 带来了持续的改进，且无需额外的适配成本。
