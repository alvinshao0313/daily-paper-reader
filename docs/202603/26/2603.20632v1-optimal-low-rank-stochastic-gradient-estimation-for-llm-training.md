...
tldr: 针对大语言模型（LLM）训练中内存受限和高维梯度噪声问题，本文提出了一种基于低秩矩阵估计的最优随机梯度估计方法。该方法利用梯度矩阵的低秩特性，通过将高维梯度投影到随机低维子空间并还原，在保持估计无偏的同时实现了最低方差。实验表明，该方法在RoBERTa微调和LLaMA预训练中显著降低了显存占用（如从16.7GB降至3.83GB），并提升了训练效果。
selection_source: fresh_fetch
motivation: 解决LLM训练中极高维参数空间带来的内存瓶颈和随机梯度噪声问题。
method: 通过求解约束泛函优化问题，设计了一种最优随机投影分布（如Haar-Stiefel投影），实现无偏且方差最小的低秩梯度估计。
result: 在RoBERTa微调中将峰值显存从16.7GB降至3.83GB，并在LLaMA预训练中表现优于传统方法。
conclusion: 该最优低秩投影策略在显著节省内存的同时，通过降低梯度方差提升了LLM的训练效率和性能。
---
