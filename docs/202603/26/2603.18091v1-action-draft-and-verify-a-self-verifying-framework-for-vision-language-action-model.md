...
## 摘要
视觉-语言-动作（VLA）模型近期在具身任务中展现出了强大的性能。现代 VLA 通常采用扩散动作专家来高效生成高精度的连续动作块，而自回归生成在底层控制方面可能较慢且精度较低。然而，自回归范式仍能提供互补的先验知识，从而提高在分布外环境中的鲁棒性和泛化能力。为了利用这两种范式，我们提出了“动作草拟与验证”（Action-Draft-and-Verify，简称 ADV）：由扩散动作专家草拟多个候选动作块，并由视觉语言模型（VLM）通过类困惑度指标在单次前向传播中对所有候选动作进行评分，从而选出其中一个。在相同的骨干网络、训练数据和动作块长度下，ADV 相比于基于扩散的基准模型，在仿真环境中的成功率提升了 4.3 个百分点，在真实世界中提升了 19.7 个百分点，且仅增加了单次 VLM 重排序的开销。
