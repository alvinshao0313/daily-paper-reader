...
motivation: 传统的梯度剪枝方法假设所有参数更新的传输和内存成本相同，忽略了硬件层面的实际能耗差异。
method: 将剪枝形式化为能耗约束的投影问题，并提出根据参数幅值与物理成本比值进行筛选的成本加权幅度剪枝（CWMP）。
result: 在非IID CIFAR-10基准测试中，CWMP在性能与能耗的帕累托前沿上始终优于传统的Top-K基准方法。
conclusion: CWMP是能耗约束投影问题的最优贪婪解，为联邦学习中实现高效的能耗感知梯度压缩提供了理论支撑。
---
