...
tags: ["query:mc"]
score: 6.0
evidence: 基于分层自回归Transformer的大语言模型架构
...
## 摘要
分词（Tokenization）是当前大语言模型（LLMs）中自然语言处理的核心组件，使模型能够将原始文本转换为可处理的单元。尽管学习型分词器被广泛采用，但它们表现出显著的局限性，包括庞大且固定的词汇表规模，以及对新领域或语言的适应性较差。我们提出了一系列基于分层自回归 Transformer（HAT）架构的模型，参数量最高达 700 亿。在 HAT 中，编码器 Transformer 将字节聚合为词嵌入，然后将其输入到主干网络（一个经典的自回归 Transformer）中。主干网络的输出随后由解码器进行交叉注意力处理，并转换回字节。我们展示了可以通过将 Llama 3.1 8B 和 70B 模型转换为 HAT 架构来重用现有的预训练模型：Llama-3.1-8B-TFree-HAT 和 Llama-3.1-70B-TFree-HAT 是字节级模型，其编码器和解码器是从头开始训练的，但我们调整了预训练的 Llama 主干网络（即移除了嵌入矩阵和输出头的 Transformer 块），以处理词嵌入而非原始 token。我们还提供了一个 7B 的 HAT 模型 Llama-TFree-HAT-Pretrained，它是在近 4 万亿个单词上完全从头开始训练的。HAT 架构通过减少所需的序列位置数量提高了文本压缩率，并增强了对词内变体（如拼写差异）的鲁棒性。通过预训练以及随后在英语和德语上的有监督微调和直接偏好优化，我们展示了在两种语言中的强大能力，在大多数基准测试中均优于原始的 Llama 3.1。我们在 Hugging Face 上发布了我们的模型（包括 200 个预训练检查点）。
