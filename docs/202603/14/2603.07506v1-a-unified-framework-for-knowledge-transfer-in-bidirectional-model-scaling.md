...
tldr: 本研究提出了BoT框架，旨在统一模型缩放中的从小到大（S2L）和从大到小（L2S）知识迁移。通过将模型权重视为连续信号，BoT利用离散小波变换（DWT）及其逆变换实现权重的上采样和下采样。该方法无需额外参数且计算高效，在DeiT、BERT和GPT等模型上显著降低了预训练开销，并达到了最先进的性能水平。
selection_source: fresh_fetch
motivation: 现有的模型缩放方法将从小到大和从大到小视为互不兼容的独立问题，缺乏统一的双向知识迁移框架。
method: 将模型权重建模为连续信号，并利用离散小波变换（DWT）及其逆变换（IDWT）实现不同尺寸模型间的权重映射。
result: "在多种架构上实现了显著的预训练计算量节省（S2L最高67.1%，L2S最高52.8%），并在GLUE和SQuAD等基准测试中表现优异。"
conclusion: BoT提供了一种高效、无参数且通用的双向模型缩放方案，为灵活的模型知识迁移开辟了新路径。
---
