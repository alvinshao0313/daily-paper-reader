...
tldr: 本研究探讨了视觉-语言-动作模型（VLA）的内部机制可解释性。针对VLA多模态及混合架构带来的复杂性，提出了特征可观测性和可控制性概念。通过线性分类器识别表征空间中的特征，并利用基于最优控制的轻量级线性干预手段，在无需微调的情况下实时引导机器人行为。实验证明该方法在多种VLA架构上能有效实现任务对齐和偏好调整，提升了具身智能的可控性。
selection_source: carryover_cache
motivation: 旨在解决大语言模型的可解释性研究因VLA模型多模态和混合架构的复杂性而难以直接迁移的问题。
method: 提出了特征可观测性与可控制性框架，通过线性分类器识别内部特征并利用最优控制理论进行轻量级线性干预。
result: 在仿真实验中证明了该方法能可靠地引导机器人行为并保持闭环性能，且无需对模型进行微调。
conclusion: VLA模型具有可解释的内部结构，支持通过在线适配实现实时任务对齐和用户偏好调整。
---
