...
tags: ["query:mc"]
score: 7.0
evidence: 评估多模态LLM中的空间表示
tldr: 本研究针对多模态大模型（MLLM）在空间推理中的稳定性问题，提出了CVT-Bench基准测试。该基准通过模拟相机轨道变换的虚拟视角变化，评估模型在不重新渲染图像情况下的关系一致性。实验发现，尽管模型在单视角任务中表现优异，但在反事实视角变换下表现出显著的不稳定性。研究还表明，增加输入表示的结构化程度（如场景图）能有效提升模型的空间推理鲁棒性。
...
result: 实验显示主流MLLM在反事实视角变换下会出现系统性性能下降，频繁违反循环一致性且关系稳定性随变换次数增加而迅速衰减。
conclusion: 单视角空间准确率高估了模型空间表示的鲁棒性，而结构化的表示形式对于提升反事实空间推理能力具有关键作用。
---

## 摘要
多模态大语言模型 (MLLMs) 在单视角空间推理任务上表现出色，但它们在反事实视角变化下是否能保持稳定的空间状态表示仍不清楚。我们引入了一个受控的诊断基准，用于在不重新渲染图像的情况下，评估假设的摄像机轨道变换下的关系一致性。通过 100 个合成场景和 6,000 个关系查询，我们测量了视角一致性、360° 循环一致性以及连续变换下的关系稳定性。尽管单视角准确率很高，但最先进的 MLLMs 在反事实视角变化下表现出系统性的性能退化，频繁违反循环一致性，且关系稳定性迅速衰减。我们进一步评估了多种输入表示（包括视觉输入、文本边界框和结构化场景图），并表明增加表示的结构性可以提高稳定性。我们的研究结果表明，单视角空间准确率高估了所诱导的空间表示的鲁棒性，且表示结构在反事实空间推理中起着至关重要的作用。
