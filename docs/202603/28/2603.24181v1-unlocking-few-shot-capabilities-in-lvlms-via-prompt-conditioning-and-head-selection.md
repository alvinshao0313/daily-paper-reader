...
tags: ["query:mc"]
score: 7.0
evidence: 解锁大型视觉语言模型的能力
tldr: 针对大型视觉语言模型（LVLM）在图像分类任务中表现不如 CLIP 的问题，本文提出了一种名为 Head Ensemble Classifiers (HEC) 的方法。该方法通过提示词调节增强特征可分性，并利用高斯判别分析筛选出最具辨别力的内部注意力头，构建无需训练的分类器。HEC 在 12 个数据集上实现了领先的零样本和少样本分类性能，弥补了 LVLM 与 CLIP 之间的差距。
selection_source: fresh_fetch
motivation: 尽管 LVLM 使用 CLIP 编码器，但在图像分类任务中表现欠佳，且其内部表征的分类潜力尚未被充分挖掘。
method: 提出 HEC 方法，通过提示词调节优化特征，并基于高斯判别分析筛选并集成最有效的视觉和文本注意力头。
result: HEC 在 12 个基准数据集上的零样本和少样本分类任务中均达到了最先进的性能水平。
conclusion: 通过合理利用 LVLM 的内部注意力头和提示词调节，可以显著提升其在传统分类任务中的表现，超越 CLIP 基准。
---
