...
tldr: 针对长视频问答中大模型上下文受限的问题，本文提出HiMu框架。它通过LLM将复杂查询分解为层级逻辑树，利用多模态专家（视觉、OCR、音频等）处理原子谓词，并结合模糊逻辑算子进行时序组合，实现精准的帧选择。该方法无需训练，在保持极高效率的同时，在Video-MME等基准测试上显著超越了现有的相似度匹配和智能体方法。
selection_source: fresh_fetch
motivation: 现有的长视频帧选择方法在简单相似度匹配（丢失结构信息）与智能体迭代推理（成本过高）之间存在严重的性能与效率权衡问题。
method: HiMu利用LLM将查询分解为逻辑树，通过多模态专家提取信号，并利用模糊逻辑算子在时序上组合这些信号以生成帧选择曲线。
result: 在Video-MME等多个榜单上，HiMu配合GPT-4o仅需16帧即可超越使用数百帧的复杂智能体系统，且计算开销降低约10倍。
conclusion: HiMu证明了通过层级化逻辑分解和多模态信号组合，可以在不增加训练成本的情况下大幅提升长视频理解的效率与准确性。
---
