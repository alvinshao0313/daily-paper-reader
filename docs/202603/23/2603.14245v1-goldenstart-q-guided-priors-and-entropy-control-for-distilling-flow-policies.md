...
tldr: 针对流匹配策略在强化学习中推理延迟高和探索效率低的问题，本文提出GSFlow。该方法通过引入基于CVAE的Q引导先验，将生成过程的起点定位在高Q值区域（即“黄金起点”），并结合显式熵正则化实现从确定性输出到随机分布的转变。实验证明，GSFlow在离线和在线连续控制任务中均显著优于现有最先进方法，有效平衡了生成效率与探索能力。
selection_source: fresh_fetch
motivation: 现有的流匹配策略蒸馏方法忽视了初始噪声分布的优化空间，且在追求快速推理时难以兼顾有效的在线探索。
method: 提出GSFlow框架，利用Q引导先验重塑生成起点，并通过熵正则化使蒸馏后的策略具备可控的随机性。
result: 在离线和在线连续控制基准测试中，该方法显著优于现有的最先进流匹配策略蒸馏技术。
conclusion: 优化生成起点和显式控制策略熵是提升蒸馏流策略效率与探索性能的关键路径。
---
