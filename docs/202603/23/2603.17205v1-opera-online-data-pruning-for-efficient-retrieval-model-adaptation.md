...
tldr: "针对稠密检索模型在特定领域微调时训练数据质量不一的问题，本文提出OPERA框架。研究发现静态剪枝虽能提升排序性能但会损害检索召回率，为此作者提出两阶段动态剪枝（DP）策略，在训练过程中自适应调整查询和文档层面的采样概率。实验表明，DP在8个数据集上显著提升了NDCG和召回率，且仅需不到50%的训练时间即可达到标准微调的效果，具有极高的效率和通用性。"
