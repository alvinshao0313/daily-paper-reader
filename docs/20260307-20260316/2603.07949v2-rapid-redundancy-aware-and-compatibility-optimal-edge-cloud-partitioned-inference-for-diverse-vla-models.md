...
motivation: 现有边缘-云端协同推理框架在处理VLA模型时易受视觉噪声干扰，且因忽视步骤间冗余而破坏了动作的物理连续性。
method: 提出RAPID框架，采用冗余感知和兼容性优化的分区策略，专门针对多样化的VLA模型进行边缘-云端协同推理优化。
result: "实验结果显示，RAPID在仅增加5%至7%额外开销的前提下，实现了高达1.73倍的推理加速。"
conclusion: RAPID通过优化边缘-云端分区推理，有效降低了VLA模型的推理延迟并保证了具身任务的动作质量。
---
