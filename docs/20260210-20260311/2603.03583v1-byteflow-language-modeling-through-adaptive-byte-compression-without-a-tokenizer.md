...
title_zh: ByteFlow：通过无分词器的自适应字节压缩进行语言建模
...
tags: ["query:mc"]
score: 7.0
evidence: 语言建模的自适应字节压缩
tldr: 针对现代语言模型依赖固定分词器导致的脆弱性，本文提出ByteFlow Net，一种无需分词器的层次化架构。该模型通过潜在表示的编码率驱动压缩，将原始字节流自适应地分割为语义单元，并利用Top-K选择保持静态计算图。实验证明，这种端到端的自适应分词策略在性能上优于传统的BPE Transformer和字节级模型，为构建更灵活、基于信息论的语言模型提供了新路径。
selection_source: fresh_fetch
motivation: 传统的固定子词分词器限制了模型的粒度灵活性，常导致推理行为脆弱且不直观。
method: 提出ByteFlow Net，利用基于编码率的压缩驱动分割技术，将字节流自适应地划分为语义单元。
result: 实验显示ByteFlow Net在性能上显著超过了基于BPE的Transformer及之前的字节级架构。
conclusion: 研究证明了端到端、无分词器建模的可行性与优越性，是迈向自适应语言模型的重要一步。
---

## 摘要
现代语言模型仍然依赖于固定的、预定义的子词分词。一旦分词器训练完成，语言模型只能在这种固定的粒度级别上运行，这往往会导致即使在强大的推理模型中也会出现脆弱且违反直觉的行为。我们引入了 ByteFlow Net，这是一种全新的层次化架构，它完全去除了分词器，转而使模型能够学习将原始字节流自行分割成具有语义意义的单元。ByteFlow Net 基于潜在表示的编码率执行压缩驱动的分段，在通过 Top-K 选择保持静态计算图的同时，产生自适应边界。与以往依赖于带有认为设计归纳偏置的脆弱启发式方法的自分词方法不同，ByteFlow Net 根据输入本身调整其内部表示粒度。实验表明，这种基于压缩的分块策略带来了显著的性能提升，ByteFlow Net 的表现优于基于 BPE 的 Transformer 和之前的字节级架构。这些结果表明，端到端的无分词器建模不仅可行，而且更加有效，为通往更具自适应性和信息基础的语言模型开辟了道路。
