...
score: 6.0
evidence: 包含量化在内的分布式优化统一压缩算法
tldr: "本文针对分布式非凸优化提出了一种统一的压缩算法框架，兼容1位压缩、饱和量化及带噪声的多种压缩器。研究在非凸设置下提供了严谨的收敛性分析，并在P-L条件下实现了线性收敛。对于局部有界压缩器，算法达到了与中心化1位压缩一致的O(1/√T)收敛率，且通过一次初始非压缩通信可提升至O(1/T^{2/3})，在多种压缩场景下均达到了当前最优水平。"
selection_source: fresh_fetch
motivation: 旨在解决分布式非凸优化中不同类型通信压缩器缺乏统一理论框架和高效收敛保证的问题。
method: 提出一种支持局部和全局有界压缩器的统一算法，并结合了误差补偿和初始非压缩通信策略。
result: "在非凸设置下实现了O(1/√T)收敛率，在P-L条件下实现线性收敛，并证明初始非压缩轮次可将收敛率优化至O(1/T^{2/3})。"
conclusion: 该算法为多种通信压缩机制提供了统一的收敛性理论支撑，在分布式非凸优化领域达到了最先进的性能指标。
---

## 摘要
本文针对分布式非凸优化提出了一种统一压缩算法，该算法适用于局部有界和全局有界通信压缩器，包括1比特压缩器、饱和量化器、具有相对和绝对压缩误差的全局有界压缩器，以及额外的任意有界噪声。我们在非凸设置下提供了严谨的收敛性分析，并在Polyak-Lojasiewicz (P-L) 条件下建立了线性收敛性。值得注意的是，在分布式非凸设置中，我们为局部有界类压缩器建立了 $\mathcal{O}(1/\sqrt{T})$ 的收敛速率，这与采用1比特压缩器的中心化算法所达到的速率相匹配，其中 $T$ 表示总迭代次数。此外，增加一轮初始的非压缩通信可将收敛速率在阶数上进一步提升至 $\mathcal{O}(1/T^{2/3})$。对于 P-L 设置和全局有界类压缩器，我们恢复了目前最先进的收敛速率。
