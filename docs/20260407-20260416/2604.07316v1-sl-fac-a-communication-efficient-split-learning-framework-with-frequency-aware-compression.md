...
motivation: 拆分学习中设备与服务器间传输的中间层激活值和梯度产生了巨大的通信瓶颈，限制了其在资源受限环境下的部署。
method: 提出由自适应频率分解（AFD）和基于频率的量化压缩（FQC）组成的框架，通过频域变换和差异化量化策略来压缩传输数据。
result: 实验结果表明，SL-FAC在大幅减少通信开销的同时，能够保持良好的模型收敛性能，展现出优异的训练效率。
