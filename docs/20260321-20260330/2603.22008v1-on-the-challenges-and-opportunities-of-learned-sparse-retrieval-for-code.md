...
## 摘要
在大规模代码库上进行检索是现代基于大语言模型（LLM）的软件工程系统的关键组成部分。现有方法主要依赖于稠密嵌入模型，而学习型稀疏检索（LSR）在代码领域仍有待深入探索。然而，由于子词碎片化、自然语言查询与代码之间的语义鸿沟、编程语言和子任务的多样性，以及代码文档的长度（这可能损害稀疏性和延迟），将稀疏检索应用于代码具有挑战性。我们推出了 SPLADE-Code，这是首个专门用于代码检索的大规模学习型稀疏检索模型系列（参数量涵盖 600M 至 8B）。尽管采用了轻量级的一阶段训练流水线，SPLADE-Code 在 1B 参数以下的检索器中仍实现了最先进的性能（在 MTEB Code 上达到 75.4），并在更大规模上取得了具有竞争力的结果（8B 参数下达到 79.0）。我们证明了学习型扩展标记对于弥合词汇与语义匹配至关重要，并提供了延迟分析，表明 LSR 能够在 100 万个段落的集合上实现亚毫秒级检索，且几乎没有效果损失。
