...
score: 8.0
evidence: 物理AI智能体与具身系统的主动推理框架
tldr: 本文从工程视角探讨了主动推理（AIF）在物理AI智能体中的应用。针对机器人等受限系统在复杂环境下的局限性，研究基于自由能原理，提出将感知、学习、规划与控制统一在变分自由能最小化框架下。通过在因子图上实现反应式消息传递，该方法支持局部并行计算，能有效应对实时性、功耗波动及环境变化，为构建具备鲁棒性和可扩展性的物理智能体提供了理论与架构支撑。
selection_source: fresh_fetch
motivation: 旨在解决物理AI智能体在资源受限且多变的现实环境中，性能远逊于生物智能体的问题。
method: 提出基于自由能原理的主动推理框架，利用因子图上的反应式消息传递实现感知与控制的统一。
result: 理论证明了该架构在处理异步数据、硬实时约束和功耗波动时具有优雅降级和在线自适应能力。
conclusion: 主动推理为构建跨尺度、同构且高效的物理智能体架构提供了一个原则性的工程基础。
---
